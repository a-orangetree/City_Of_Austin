...
coef(regsubset_model, 6)
coef(regsubset_model, 7)


####### Ridge Regression ################


ridge_model <- glmnet(crimes_2016 ~ crimes_2015 + Unemployment + Hispanic + Corridor + Residential
                      , data = train_data
                      , alpha = 0)

ridge_cv <- cv.glmnet(crimes_2016 ~ crimes_2015 + Unemployment + Hispanic + Corridor + Residential
                      , data = train_data
                      , alpha = 0)
...
lasso_model <- glmnet(crimes_2016 ~crimes_2015 + Unemployment + Hispanic + Corridor + Residential
                      , data = train_data
                      , alpha = 1)

lasso_cv <- cv.glmnet(crimes_2016 ~ crimes_2015 + Unemployment + Hispanic + Corridor + Residential
                      , data = train_data
                      , alpha = 1)
    ...
    for (n_pred in mtries) {
      for (min_obs in min_observations) {
        for (term_nodes in terminal_nodes) {
          ...
          rf_model <- randomForest(crimes_2016 ~ crimes_2015 + Unemployment + Hispanic + Corridor + Residential
                                   , data = train_data
                                   , ntrees = tree_size
                                   , mtry = n_pred
                                   , nodesize = min_obs
...
x <- select(train_data, crimes_2015, Unemployment,  Hispanic,  Corridor, Residential) 
